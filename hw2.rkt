...
(define (f1 x) (* x x)) ;f1 returns #<procedure:f1>
(define (f2) (lambda (x) (* x x))) ;(f2) returns #<procedure>
(define (f3 x) (* x x)) ;(f3 3) returns 9
(define (f4) (lambda () (f4))) ;((f4)) returns #<procedure>
(define (f5) (lambda () (lambda (x) (* x x)))) ;(((f5)) 3) returns 9
...
(define (estimate-pi)
  (exact->inexact (/ (* 8 (product (lambda (x) (* x x)) 4 (lambda (x) (+ x 2)) 100000) 100002)
     (product (lambda (x) (* x x)) 3 (lambda (x) (+ x 2)) 100001)))
)
...
(define (prod-of-some-numbers n)
  (filtered-accumulate * 1 print 1 (lambda (x) (+ x 1)) n rel-prime?)
)

; SICP 1.40 - Define cubic

(define (cubic a b c)
  (lambda (x) (+ ((lambda (x) (* x x x)) x) (* a ((lambda (x) (* x x)) x)) (* b x) c))
)
...
(define (my-repeated proc n)
  (if (equal? n 0)
      (lambda (x) x)
      (comp proc (my-repeated proc (- n 1))))
)
